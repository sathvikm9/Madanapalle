...
Sai Chitra uses District for its accurate live seat layout. For an 11:00 AM show it opens the exact District session at 11:10:05 AM and retries through 11:14:05 AM, waits for the two-class seat grid to stabilize, and counts each live available or occupied seat. The route must prove the exact Sai Chitra cinema and session before the Worker accepts it. TicketNew is retained only as an ESTIMATED summary fallback when every District live attempt fails; any verified District live snapshot outranks every summary estimate.
